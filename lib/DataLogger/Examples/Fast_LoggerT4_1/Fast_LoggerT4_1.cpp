...
void loop() {
  ...
  TLoggerStat *tsp;
  tsp =  mydl.GetStatus();
  // can't depend on tsp->islogging---it doesn't quite until just a few MB left!
  ...
  if (Serial.available()) {
    ...
    if (ch == 'q') QuitLogging();
    if (ch == 's') GetStatus();
    if (ch == 'f') mydl.FormatSD(false); // format exFat for > 32GB
...
void StartLogging(void) {
  ...
  numrecs = 0;
  mydl.StartLogger("", 1000);  // sync once per second
